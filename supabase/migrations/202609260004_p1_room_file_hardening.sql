-- P1 attachment hardening: registered-object reads, org-scoped version chains,
-- idempotency conflict detection, stronger path/name validation, and captions.

alter table public.room_file_versions
  drop constraint if exists room_file_versions_room_id_logical_name_version_no_key;

alter table public.room_file_versions
  add constraint room_file_versions_room_org_name_version_key
  unique (room_id, uploader_organization_id, logical_name, version_no);

create index if not exists room_file_versions_room_org_name_idx
  on public.room_file_versions(room_id, uploader_organization_id, logical_name, version_no desc);
...
drop function if exists public.register_room_file_version(
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(
      p_room_id::text || chr(0) || p_actor_organization_id::text || chr(0) || v_logical_name,
      0
    )
  );
